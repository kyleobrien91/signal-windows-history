...
def inspect_video_duration(enc_path: str, local_key: str, size: int) -> float:
    """Fast in-memory header inspection to retrieve video duration in seconds."""
    if not os.path.exists(enc_path) or av is None or size <= 0:
        return 0.0
    try:
        from crypto.attachment import stream_attachment_range
        chunk_size = min(size - 1, 1024 * 1024)
        first_chunk = b"".join(stream_attachment_range(enc_path, local_key, 0, chunk_size))
        container = av.open(io.BytesIO(first_chunk))
