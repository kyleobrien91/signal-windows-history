...
class DerivedMediaCache:
    ...
    def _is_corrupt_error(self, err: Exception) -> bool:
        ...
        return any(term in msg for term in (
            "corrupt", "malformed", "not a database", "file is not a database",
            "integrity check failed", "tree page", "index inconsistency"
        ))
    ...
    def _sanitize_params(self, params: Optional[dict]) -> dict:
        """Replaces sensitive key material like src_key/localKey with HMAC digests."""
        if not params:
            return {}
        clean = dict(params)
        if 'src_key' in clean:
            raw_key = str(clean.pop('src_key'))
            clean['src_key_hash'] = hmac.new(self._master_key, f"srckey|{raw_key}".encode('utf-8'), hashlib.sha256).hexdigest()
        return clean
    ...
    def derive_cache_key(
        ...
        Ensures raw attachment paths, filenames, IDs, or keys never appear in cache keys, index DB, or filenames.
        """
        clean_params = self._sanitize_params(params)
        sorted_params = json.dumps(clean_params, sort_keys=True, separators=(',', ':'))
    ...
    def put(
        ...
        if enc_size <= self.l2_max_bytes:
            ...
            # Insert/update in SQLite index using opaque attachment_key and sanitized params
