    ...
    with _db_lock:
        ...
        while len(filtered_media) < limit + 1:
            ...
            for r in rows:
                ...
                # Extract duration on initial catalogue query if not yet populated
                duration = meta.get("duration", 0)
                if duration <= 0 and path and local_key:
                    try:
                        import os
                        from crypto.derivatives import inspect_video_duration
                        attach_root = os.path.join(os.environ.get("APPDATA", ""), "Signal", "attachments.noindex")
                        enc_path = os.path.join(attach_root, path)
                        dur = inspect_video_duration(enc_path, local_key, size)
                        if dur > 0:
                            duration = dur
                            from metadata import _set_meta
                            _set_meta(item_id, duration=dur)
                    except Exception:
                        pass
